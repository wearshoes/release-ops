...
声明 `buildHook` 时，其 `script` 必须位于 `runtimeFiles`，并导出 provider/v2 的标准异步接口
`planBuildHook(config, context)` 与 `runBuildHook(plan, options)`。核心只按已启用 provider 的 manifest
动态加载该模块；模块缺失或接口不完整会让 audit 失败。provider 不得要求核心按 id 硬编码调用。
