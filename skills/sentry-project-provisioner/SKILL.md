...
3. Run `node ../../scripts/sentry-project.mjs --help`, inspect, and dry-run before creation. Create only after authorization of the exact organization, team, project slug, platform, and API base. Existing project identity makes this step read-only.
4. If project creation is required and the administration token is absent, pause for a credential handoff or for the user to sign in to the same Chrome session. Confirm the exact organization, team, and project slug before any creation. Never ask the user to transmit a persistent credential through conversation.
...
9. Rerun the checker and require `configured`, then audit the four credential roles by name and purpose without reading values.
